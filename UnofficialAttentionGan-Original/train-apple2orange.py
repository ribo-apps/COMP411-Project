...
train_imagefolder = dataloader.PairImageFolder('/datasets/CycleGan/apple2orange',
                                              train_transforms,
                                              mode='train')
...
def save(path, netG_A2B, netG_B2A, netD_A, netD_B, optimizer_G, optimizer_D, epoch):
    ...
    torch.save(state, '/kuacc/users/ebostanci18/comp411/AttentionGan-SelfAttention/checkpoint/'+path)
    print("Done!")

def load(netG_A2B, netG_B2A, netD_A, netD_B, optimizer_G, optimizer_D):
    print('Loading...', end=' ')
    checkpoint = torch.load('/kuacc/users/ebostanci18/comp411/AttentionGan-SelfAttention/checkpoint/apple2orange/ckpt.pth')
    ...
    for i, data in enumerate(train_loader, 0):                  
        ...
        if i % 1000 == 0:
            with torch.no_grad():
                ...
                print()
                save('apple2orange/ckpt.pth', netG_A2B, netG_B2A, netD_A, netD_B, optimizer_G, optimizer_D, current_epoch)
