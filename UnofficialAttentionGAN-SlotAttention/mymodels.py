...
import os
import dataloader
from slot_attention import model
...
class Generator(nn.Module):
    def __init__(self, input_nc, output_nc, num_att, ngf=64, n_middle=9, norm='IN', activation='relu', pretrained=False):
        ...
        res_size = 128
        self.slot_att = model.SlotAttentionModel(
            ...
            kernel_size= 5,
            slot_size= res_size,
            hidden_dims = (64, 64, 64, res_size),
            decoder_resolution = (res_size//16, res_size//16),
            empty_cache=False,
        )
    ...
    def forward(self, inputs):
        ...
        #att = self.att(img)
        #print("f", img.shape, att.shape)
        recon_combined, recons, masks, slots = self.slot_att(img)
        #print("fs",masks.shape, slots.shape, masks.squeeze(2).shape)
        att = masks.squeeze(2)
        att_list = []
        content_list = []
        ...
        #print("f2", len(content_list), len(att_list))
